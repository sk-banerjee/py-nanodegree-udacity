...
def print_results(results_dic, results_stats_dic, model, 
    ...
    for key,value in results_stats_dic.items():
        if key.startswith('pct_'):
            msg = key.replace('pct_', '% ').replace('_',' ').title()
            print("{}: {}".format(msg,value))
        
    if print_incorrect_dogs:
        dogs_misclassification = 0
        if ((results_stats_dic['n_correct_dogs'] + results_stats_dic['n_correct_notdogs']) != results_stats_dic['n_images']):
            for key,value in results_dic.items():
                if value[3] == 1 and value[4] == 0:
                    dogs_misclassification += 1
        print("{} dog(s) misclassified".format(dogs_misclassification))
        
    if print_incorrect_breed:
        breeds_misclassified = 0
        if results_stats_dic['n_correct_dogs'] != results_stats_dic['n_correct_breed']:
            for key,value in results_dic.items():
                if sum(value[3:]) == 2 and value[2] == 0:
                    breeds_misclassified += 1
        print("{} breed(s) misclassified".format(breeds_misclassified))
